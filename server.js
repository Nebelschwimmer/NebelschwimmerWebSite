...
const singleTextSchema = new mongoose.Schema({
  name: String,
  content_en: String,
  content_ru: String,
  likes: [String],
  comments: [{userId: String, comment_body: String}]

});
const SingleText = mongoose.model('singleText', singleTextSchema);

const singleText = new SingleText({
  name: 'Вася Пупкин',
  content_en: 'Test',
  content_ru: 'Тест',
  likes: [],
  comments: [{userId: '', comment_body:''}]
})

// singleText.save().then(
//         () => console.log("One text added"), 
//         (err) => console.log(err)
//     );

const TextsSchema = new mongoose.Schema({
texts: [singleTextSchema]
})
...
app.get('/textsTest', (req, res) => {
  SingleText.find({}).then(function (texts) {
    res.send(texts);
});
})
...
app.use(express.json());
  ...
  let randomTrackID = "id_" + Math.random().toString(16).slice(2)
   ...
   // --------Сommon vars for texts API------------- 
  const readTextsJSON = fs.readFileSync('./texts.json', "utf8")  
